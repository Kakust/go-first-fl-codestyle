...
func defence(charName, charClass string) string {
	var damageBlocked int

	switch charClass {
	case "warrior":
		damageBlocked = 10 + randint(5, 10)
	case "mage":
		damageBlocked = 10 + randint(-2, 2)
	case "healer":
		damageBlocked = 10 + randint(2, 5)
	default:
		return "неизвестный класс персонажа"
	}

	return fmt.Sprintf("%s блокировал %d урона.", charName, damageBlocked)
}
